...
def generate_launch_description():
    launch_file_dir = os.path.join(get_package_share_directory('labs'), 'launch')
    pkg_gazebo_ros = get_package_share_directory('gazebo_ros')
    turtle_launch_file_dir = os.path.join(get_package_share_directory('turtlebot3_gazebo'), 'launch')

    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
    x_pose = LaunchConfiguration('x_pose', default='1.4')
    y_pose = LaunchConfiguration('y_pose', default='7.7')
    z_pose = LaunchConfiguration('z_pose', default='-0.8')

    world = os.path.join(
        get_package_share_directory('labs'),
        'worlds',
        'spot_gazebo_two.world'
    )
